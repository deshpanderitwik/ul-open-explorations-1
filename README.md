...
A static dashboard on **GitHub Pages** lets you watch progress without reading
code: the current champion's numbers, the ladder, fitness over builds, recent
runs, and recent activity.

**Live:** https://deshpanderitwik.github.io/ul-open-explorations-1/

It's served straight from the branch (this repo uses Pages' "deploy from branch"
mode, so a root `.nojekyll` disables Jekyll and the root `index.html` redirects to
`dashboard/public/`). The data it reads, `dashboard/public/data.json`, is
generated by `dashboard/build.py` from the repo's own records
(`archive/runs.jsonl`, `spec/ladder/ladder.json`, git log) and committed — so
regenerate it when those change:

```sh
python dashboard/build.py        # refresh dashboard/public/data.json, then commit
# preview locally:
(cd dashboard/public && python -m http.server 8765)   # open http://localhost:8765
```
